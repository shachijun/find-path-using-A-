...
class TendTo:
    ...
    def imaginemove(self): #put in open list and close list, close list memerize parent
        ...
        ### expanded_cell_array = [] append starty start x to it
        gva = 1
        ...
        List = sorted(sorted(List, key=lambda listo: listo[1], reverse=False), key=lambda listo: listo[0])  # sort by age
        ...
        while stepx != environment.gx or stepy != environment.gy: ## != environment.sx sy
            ...
            if len(List) == 0:
                MOVE=[]
                return MOVE, self.Step, False
            List = sorted(sorted(List, key=lambda listo: listo[1], reverse=False), key=lambda listo: listo[0])  # sort by f value
            locat = List.pop(0)
            stepx, stepy = locat[3], locat[4]
